...
/*        for(int idx=0; idx<tree.size(); idx++)
    ...
    template<typename PointDatatype>
    void get_nearest_neighbor_kernel(Node<PointDatatype>* device_tree, Point2D<int> query_point, int total_tree_level,
...
/*        for(int idx=0; idx<vertex_count; idx++)
    ...
    template<typename PointDatatype>
    float euclidean_distance(Point3D<PointDatatype> pointa, Point3D<PointDatatype> pointb){
        return sqrtf(powf(pointa.x - pointb.x, 2) + powf(pointa.y - pointb.y, 2) + powf(pointa.z - pointb.z, 2));
    }

    template<typename Point>
    int get_nearest_neighbor(Node<Point>* device_tree, Point query_point, int total_tree_levels, int dimensions) {
        int level = 0;
